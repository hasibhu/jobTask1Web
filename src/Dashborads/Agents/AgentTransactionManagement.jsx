...
const AgentTransactionManagement = () => {
  ...
  const handleAccept = async (request) => {
    try {
      ...
      const userResponse = await axiosPublic.get(
        `/users/phone/${request.userPhoneNumber}`
      );
      ...
      if (request.type === "CashIn") {
        ...
        // Update balances
        await axiosPublic.patch(`/users/${agent._id}/balance`, {
          balance: agentBalance - parseInt(request.amount),
        });
        await axiosPublic.patch(`/users/${user._id}/balance`, {
          balance: userBalance + parseInt(request.amount),
        });
        ...
        if (
          agentBalance < parseInt(request.amount) ||
          userBalance < parseInt(request.amount)
        ) {
          Swal.fire({
            icon: "error",
            title: "Insufficient Balance",
  ...
  if (!requests) {
    return (
      <div className="flex justify-center items-center text-7xl font-extrabold text-red-600 mt-32 ">
  ...
  return (
    <div className="bg-gradient-to-b from-fuchsia-900 to-slate-800 lg:w-[576px] mx-auto rounded-xl text-white pb-10">
      ...
      <h1 className="text-center text-2xl font-serif mb-4">
